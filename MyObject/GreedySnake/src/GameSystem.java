...
public class GameSystem extends JFrame {
    ...
    JPanel checkerboard;
    Body snakeBody;
    double frame = 23.3; //fps
    int snakeSpeed = 20; //blocks
            ...
            public void keyPressed(KeyEvent e) {
                ...
                switch(e.getKeyCode()) {
                    case KeyEvent.VK_UP : // 38,
                        if(snakeBody.getMoveDirection() == Direction.DOWN)
                            break;
                        snakeBody.setMoveDirection(Direction.UP);
                        break;
                    case KeyEvent.VK_DOWN :
                        if(snakeBody.getMoveDirection() == Direction.UP)
                            break;
                        snakeBody.setMoveDirection(Direction.DOWN);
                        break;
                    case KeyEvent.VK_LEFT :
                        if(snakeBody.getMoveDirection() == Direction.RIGHT)
                            break;
                        snakeBody.setMoveDirection(Direction.LEFT);
                        break;
                    case KeyEvent.VK_RIGHT :
                        if(snakeBody.getMoveDirection() == Direction.LEFT)
                            break;
